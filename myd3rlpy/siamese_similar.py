...
def compute_vector_norm(tensor1, tensor2):
    ...
    return ret
def similar_euclid(obs_all, obs_transition, dataset_name, indexes_name, distances_name, input_indexes=None, eval_batch_size=1000, topk=64, compare_dim=2, device=device):
    near_indexes_ = []
    near_distances_ = []
    i = 0
    print(f'obs_all: {obs_all.shape}')
        ...
        if i + eval_batch_size < obs_all.shape[0]:
            while j < obs_transition.shape[0]:
                if j + eval_batch_size < obs_transition.shape[0]:
                    tensor1 = torch.from_numpy(obs_all[i: i + eval_batch_size, :compare_dim]).to(device).unsqueeze(dim=1).expand(-1, eval_batch_size, -1)
                    tensor2 = torch.from_numpy(obs_transition[j: j + eval_batch_size, :compare_dim]).to(device).unsqueeze(dim=0).expand(eval_batch_size, -1, -1)
                    siamese_distance = compute_vector_norm(tensor1, tensor2).cpu()
                    in_results.append(siamese_distance)
                else:
                    tensor1 = torch.from_numpy(obs_all[i: i + eval_batch_size, :compare_dim]).to(device).unsqueeze(dim=1).expand(-1, obs_transition.shape[0] - j, -1)
                    tensor2 = torch.from_numpy(obs_transition[j:, :compare_dim]).to(device).unsqueeze(dim=0).expand(eval_batch_size, -1, -1)
                    siamese_distance = compute_vector_norm(tensor1, tensor2).cpu()
                    in_results.append(siamese_distance)
                j += eval_batch_size
        else:
            while j < obs_transition.shape[0]:
                if j + eval_batch_size < obs_transition.shape[0]:
                    tensor1 = torch.from_numpy(obs_all[i:, :compare_dim]).to(device).unsqueeze(dim=1).expand(-1, eval_batch_size, -1)
                    tensor2 = torch.from_numpy(obs_transition[j: j + eval_batch_size, :compare_dim]).to(device).unsqueeze(dim=0).expand(obs_all.shape[0] - i, -1, -1)
                    siamese_distance = compute_vector_norm(tensor1, tensor2).cpu()
                    in_results.append(siamese_distance)
                else:
                    tensor1 = torch.from_numpy(obs_all[i:, :compare_dim]).to(device).unsqueeze(dim=1).expand(-1, obs_transition.shape[0] - j, -1)
                    tensor2 = torch.from_numpy(obs_transition[j:, :compare_dim]).to(device).unsqueeze(dim=0).expand(obs_all.shape[0] - i, -1, -1)
                    siamese_distance = compute_vector_norm(tensor1, tensor2).cpu()
                    in_results.append(siamese_distance)
                j += eval_batch_size
        print(f'i: {i}')
...
def similar_mb_l2(obs_all, obs_transition, indexes_euclid=None, eval_batch_size=10000, topk=256, compare_dim=2):
    results = []
    near_distances = []
    if len(obs_transition.shape) == 2:
        ...
        while j < obs_transition.shape[0]:
            if j + eval_batch_size < obs_transition.shape[0]:
                tensor1 = obs_all[:, :compare_dim].unsqueeze(dim=1).expand(-1, eval_batch_size, -1)
                tensor2 = obs_transition[j: j + eval_batch_size, :compare_dim].unsqueeze(dim=0).expand(obs_all.shape[0], -1, -1)
                siamese_distance = torch.linalg.vector_norm(tensor1 - tensor2, dim = 2)
                in_results.append(siamese_distance)
            else:
                tensor1 = obs_all[:, :compare_dim].unsqueeze(dim=1).expand(-1, obs_transition.shape[0] - j, -1)
                tensor2 = obs_transition[j:, :compare_dim].unsqueeze(dim=0).expand(obs_all.shape[0], -1, -1)
                siamese_distance = torch.linalg.vector_norm(tensor1 - tensor2, dim = 2)
                in_results.append(siamese_distance)
            j += eval_batch_size
        in_results = torch.cat(in_results, dim=1)
...
def similar_mb_feature(obs_all, obs_transition, impl, indexes_euclid=None, eval_batch_size=10000, topk=256, compare_dim=2):
    results = []
    near_distances = []
    if len(obs_transition.shape) == 2:
        in_results = []
        ...
        while j < obs_transition.shape[0]:
            if j + eval_batch_size < obs_transition.shape[0]:
                tensor1 = obs_all[:, :compare_dim].unsqueeze(dim=1).expand(-1, eval_batch_size, -1)
                tensor1_feature = impl._policy._encode(tensor1)
                tensor2 = obs_transition[j: j + eval_batch_size, :compare_dim].unsqueeze(dim=0).expand(obs_all.shape[0], -1, -1)
                tensor2_feature = impl._policy._encode(tensor2)
                siamese_distance = torch.linalg.vector_norm(tensor1_feature - tensor2_feature, dim = 2)
                in_results.append(siamese_distance)
            else:
                tensor1 = obs_all[:, :compare_dim].unsqueeze(dim=1).expand(-1, obs_transition.shape[0] - j, -1)
                tensor1_feature = impl._policy._encode(tensor1)
        ...
        for i in range(obs_transition.shape[0]):
            ...
            tensor2 = obs_transition[i, :, :compare_dim]
            tensor2_feature = impl._policy._encode(tensor2)
            siamese_distance = torch.linalg.vector_norm(tensor1_feature - tensor2_feature, dim = 1)
            near_distance, near_indexes = torch.topk(siamese_distance, topk, largest=False)
            results.append(near_indexes)
            near_distances.append(near_distance)
    near_indexes = torch.cat(results, dim=0).to(torch.int64)
