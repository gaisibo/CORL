...
from myd3rlpy.dataset cimport CTransition
from libc.string cimport memcpy, memset
from libcpp cimport bool, nullptr
from libcpp.memory cimport make_shared, shared_ptr

from d3rlpy.logger import LOG
from d3rlpy.dataset import MDPDataset as OldMDPDataset, Episode as OldEpisode
#from d3rlpy.dataset import Transition as OldTransition, TransitionMiniBatch as OldTransitionMiniBatch
...
def _rewards_to_rtgs(rewards, terminals, gamma = 0.99):
    start = 0
    rtgs = []
    for i in range(_safe_size(rewards)):
        if terminals[i] == 1 or i == _safe_size(rewards) - 1:
            end = i + 1
            gammas = np.ones((end - start), ) * gamma
            gammas[0] = 1
            gammas = np.cumprod(gammas, axis=0)
            part_rtgs = np.flip(rewards[start: end], axis=0)
            part_rtgs = np.multiply(gammas, part_rtgs)
            part_rtgs = np.cumsum(part_rtgs, axis=0)
            part_rtgs = np.flip(part_rtgs, axis=0)
            rtgs.append(part_rtgs)
            start = end
...
class MDPDataset(OldMDPDataset):
    ...
    def __init__(
        ...
        self._rtgs = _rewards_to_rtgs(self._rewards, np.logical_or(self._terminals, self._episode_terminals), gamma=gamma)
    ...
    def compute_stats(self):
        ...
        stats = {
            ...
            'rtgs': {
                'mean': np.mean(self._rtgs),
                'std': np.std(self._rtgs),
                'min': np.min(self._rtgs),
                'max': np.max(self._rtgs),
                'histogram': np.histogram(self._rtgs, bins=20)
            }
        }
    ...
    def append(
        ...
        # append rests
        rtgs = _rewards_to_rtgs(rewards, terminals)
        self._rewards = np.hstack([self._rewards, rewards])
        self._rtgs = np.hstack([self._rtgs, rtgs])
    ...
    def dump(self, fname):
        ...
        with h5py.File(fname, 'w') as f:
            ...
            f.create_dataset('rtgs', data=self._rtgs)
...
class Episode(OldEpisode):
    ...
    def __init__(
        ...
        self._rtgs = np.asarray(rtgs, dtype=np.float32)
...
cdef class TransitionMiniBatch():
    ...
    cdef void _assign_to_batch(
        ...
        rtgs_ptr[batch_index] = ptr.get().rtg
    ...
    @property
    def rtgs(self):
        """ Returns mini-batch of rtgs at `t`.

        Returns:
            numpy.ndarray: rtgs at `t`.
