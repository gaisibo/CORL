...
from myd3rlpy.siamese_similar import similar_mb, similar_mb_l2, similar_mb_feature
...
class CO():
    ...
    * TD3 updates the policy function after several Q function updates in order
    ...
    def online_fitter(
        ...
        for total_step in xrange(1, n_steps + 1):
            ...
            with logger.measure_time("step"):
                ...
                # print(f"update_start_step={update_start_step}")
                # print(f"len(buffer)={len(buffer)}")
                # print(f"self._batch_size={self._batch_size}")
                # print(f"update_interval={total_step}")
                if total_step > (update_start_step if not test else 0) and len(buffer) > self._batch_size:
                    if total_step % update_interval == 0:
                        ...
                        # update parameters
                        with logger.measure_time("algorithm_update"):
                            loss = self.update(batch, replay_batches, online=True)
            ...
            # # save model parameters
            # if epoch % save_interval == 0 and (total_step + 1) % n_steps_per_epoch == 0:
                logger.save_model(total_step, self)
    ...
    def evaluate(self,
        ...
        # save metrics
        metrics = logger.commit("finish", 0)
    ...
    def generate_replay_data_trajectory(self, dataset, episodes, start_index, max_save_num=1000, max_export_time=10, max_export_step=1000, real_action_size=1, real_observation_size=1, n_epochs=None, n_steps=500000,n_steps_per_epoch=5000, shuffle=True, test=False, indexes_euclid=None):
        assert self._impl is not None
                        ...
                        if 'prob' in self._experience_type:
                            ...
                            if indexes_euclid is not None:
                                if self._distance_type == 'l2':
                                    near_indexes, _ = similar_mb_l2(start_next_observations, near_next_observations, topk=1)
                                elif self._distance_type == 'feature':
                                    near_indexes, _ = similar_mb_feature(start_next_observations, near_next_observations, topk=1)
                                else:
                                ...
                                while batch_idx + eval_batch_size < observations.shape[0]:
                                    if self._distance_type == 'l2':
                                        near_indexes_, near_distances_ = similar_mb_l2(start_next_observations, near_next_observations_list[idx].to(self._impl.device), topk=1)
                                    elif self._distance_type == 'feature':
                                        near_indexes_, near_distances_ = similar_mb_feature(start_next_observations, near_next_observations_list[idx].to(self._impl.device), self._impl, topk=1)
    ...
    def generate_replay_data_transition(self, dataset, max_save_num=1000, start_num=50, real_observation_size=1, real_action_size=1, batch_size=16, indexes_euclid=None, distances_euclid=None, d_threshold=None):
        with torch.no_grad():
            if isinstance(dataset, MDPDataset):
    ...
    def generate_replay_data_episode(self, dataset, all_max_save_num=1000, max_export_step=1000, start_num=1, real_observation_size=1, real_action_size=1, batch_size=16, test=False, indexes_euclid=None):
        ...
        with torch.no_grad():
            ...
            #     return self.generate_replay_data_trajectory(dataset, episodes, start_index, max_save_num=max_save_num, max_export_step=max_export_step, real_observation_size=real_observation_size, real_action_size=real_action_size, with_generate=with_generate, test=test, indexes_euclid=indexes_euclid)
    ...
    def generate_replay(self, task_id, origin_dataset, env, replay_type, experience_type, replay_datasets, save_datasets, max_save_num, max_export_step, real_action_size, real_observation_size, indexes_euclid, distances_euclid, d_threshold, test, model_path, algos_name, learned_tasks):

        start_time = time.perf_counter()
        if replay_type in ['ewc', 'si', 'rwalk', 'fix', 'none'] or experience_type in ['none', 'all']:
            ...
            replay_datasets[task_id], save_datasets[task_id] = self.generate_replay_data_transition(origin_dataset, max_save_num=max_save_num, real_action_size=real_action_size, real_observation_size=real_observation_size, indexes_euclid=indexes_euclid, distances_euclid=distances_euclid, d_threshold=d_threshold)
            ...
            replay_datasets[task_id], save_datasets[task_id] = self.generate_replay_data_episode(origin_dataset, all_max_save_num=max_save_num, max_export_step=max_export_step, real_action_size=real_action_size, real_observation_size=real_observation_size, test=test, indexes_euclid=indexes_euclid)
            print(f"len(replay_datasets[task_id]): {len(replay_datasets[task_id])}")
        elif experience_type == 'generate':
            replay_datasets[task_id], save_datasets[task_id] = self.generate_new_data_replay(origin_dataset, max_save_num=max_save_num, real_observation_size=real_observation_size, real_action_size=real_action_size)
            print(f"len(replay_datasets[task_id]): {len(replay_datasets[task_id])}")
        elif experience_type in ['model', 'model_next', 'model_this', 'model_prob']:
            episodes = origin_dataset.episodes
            episode_num = 0
            saved = False
            select_num = 0
            start_index = 0
            ...
            if not saved:
                start_index = episode_num
            replay_datasets[task_id], save_datasets[task_id] = self.generate_replay_data_trajectory(origin_dataset, episodes, start_index, max_save_num=max_save_num, max_export_step=max_export_step, real_observation_size=real_observation_size, real_action_size=real_action_size, test=test, indexes_euclid=indexes_euclid)
        ...
        #mean_eval_episodes = [origin_dataset for task_id in learned_tasks]
        #mean_inner_qs = []
        #mean_outer_qs = []
        #for mean_scorer, mean_eval_episode in zip(mean_scorers, mean_eval_episodes):
        #    mean_inner_q, mean_outer_q = mean_scorer(self, mean_eval_episode)
        #    mean_inner_qs.append(mean_inner_q)
        #    mean_outer_qs.append(mean_outer_q)
        #replay_inner_qs = []
        #replay_outer_qs = []
        #for n in learned_tasks[:-1]:
        #    if save_datasets[n] is not None:
        #        replay_inner_q, replay_outer_q = q_replay_scorer(real_action_size=real_action_size, test_id=str(n))(self, save_datasets[n])
        #        replay_inner_qs.append(replay_inner_q)
        #        replay_outer_qs.append(replay_outer_q)
        #replay_inner_qs.append(0)
        #replay_outer_qs.append(0)
        #no_replay_inner_qs = [(mean_q * origin_dataset.observations.shape[0] - max_save_num * replay_q) / (origin_dataset.observations.shape[0] - max_save_num) for mean_q, origin_dataset, replay_q in zip(mean_inner_qs, origin_datasets.values(), replay_inner_qs)]
        #no_replay_outer_qs = [(mean_q * origin_dataset.observations.shape[0] - max_save_num * replay_q) / (origin_dataset.observations.shape[0] - max_save_num) for mean_q, origin_dataset, replay_q in zip(mean_outer_qs, origin_datasets.values(), replay_outer_qs)]
        #for i, (mean_q, replay_q, no_replay_q) in enumerate(zip(mean_inner_qs, replay_inner_qs, no_replay_inner_qs)):
        #    print(f'final test {i} inner mean_q: {mean_q}; inner replay_q: {replay_q}; inner no_replay_q : {no_replay_q}. ', end='')
        #for i, (mean_q, replay_q, no_replay_q) in enumerate(zip(mean_outer_qs, replay_outer_qs, no_replay_outer_qs)):
        #    print(f'final test {i} outer mean_q: {mean_q}; outer replay_q: {replay_q}; outer no_replay_q : {no_replay_q}. ', end='')
        #print()

    def calc_layer_mean_grad(self, actor_grad_save, clone_actor_grad_save):
        mean_actor_grad = []
        i = 0
        index = 0
        old_index = 0
        for name, param in self._impl._policy.named_parameters():
            if str(i) in name:
                index += param.numel()
            else:
                mean_actor_grad.append(torch.mean(actor_grad_save[old_index: index]).detach().cpu().numpy())
                old_index = index
                i += 1

        if hasattr(self._impl, "_clone_policy") and clone_actor_grad_save is not None:
            mean_clone_actor_grad = []
            i = 0
            index = 0
            old_index = 0
            for name, param in self._impl._clone_policy.named_parameters():
                if str(i) in name:
                    index += param.numel()
                else:
                    mean_clone_actor_grad.append(torch.mean(clone_actor_grad_save[old_index: index]).detach().cpu().numpy())
                    old_index = index
                    i += 1
        else:
            mean_clone_actor_grad = None
        return mean_actor_grad, mean_clone_actor_grad
