    ...
    * TD3 updates the policy function after several Q function updates in order
    ...
    def _create_impl(
        ...
        if self._impl_name == 'sac':
            from myd3rlpy.algos.torch.co_sac_impl import COSACImpl as COImpl
        elif self._impl_name == 'sacn':
            from myd3rlpy.algos.torch.co_sacn_impl import COSACNImpl as COImpl
        elif self._impl_name == 'cql':
            _impl_dict['alpha_optim_factory'] = self._alpha_optim_factory
            _impl_dict['alpha_learning_rate'] = self._alpha_learning_rate
            _impl_dict['alpha_threshold'] = self._alpha_threshold
            _impl_dict['initial_alpha'] = self._initial_alpha
    ...
    def update(self, batch: TransitionMiniBatch, replay_batches: Optional[Dict[int, List[Tensor]]]=None, online: bool = False) -> Dict[int, float]:
    ...
    def _update(self, batch: TransitionMiniBatch, replay_batches: Optional[Dict[int, List[Tensor]]]=None) -> Dict[int, float]:
        ...
        if self._impl_name == 'cql' and self._alpha_learning_rate > 0:
            alpha_loss, alpha = self._impl.update_alpha(batch)
            metrics.update({"alpha_loss": alpha_loss, "alpha": alpha})

        if not self._replay_critic:
            critic_loss, _, _, _ = self._impl.update_critic(batch)
            metrics.update({"critic_loss": critic_loss})
        else:
            critic_loss, replay_critic_loss, replay_clone_loss, _ = self._impl.update_critic(batch, replay_batches)
            metrics.update({"critic_loss": critic_loss})
            metrics.update({"replay_critic_loss": replay_critic_loss})
            metrics.update({"replay_critic_clone_loss": replay_clone_loss})
        ...
        metrics.update({"replay_actor_clone_loss": replay_clone_loss})
