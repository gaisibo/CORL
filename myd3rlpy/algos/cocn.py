...
class CO():
    ...
    * TD3 updates the policy function after several Q function updates in order
    ...
    def generate_replay_data_trajectory(self, dataset, episodes, start_index, max_save_num=1000, random_save_num=1000, max_export_time=10, max_export_step=300, real_action_size=1, real_observation_size=1, n_epochs=None, n_steps=500000,n_steps_per_epoch=5000, shuffle=True, with_generate='generate_model', test=False, indexes_euclid=None):
        assert self._impl is not None
        ...
        while len(orl_indexes_all) < max_save_num:
            max_export_step = min(max_export_step + 100, 1000)
            for orl_indexes, orl_ns in zip(orl_indexes_list, orl_ns_list):
                ...
                while export_step < max_export_step:
                    ...
                    if 'noise' in self._sample_type:
                        if max_export_step < 1000:
                            model_noise = self._model_noise * 0.1
                        else:
                            model_noise = self._model_noise
                        noise = model_noise * max(1, (export_time / max_export_time)) * torch.randn(start_actions.shape, device=self._impl.device)
                        start_actions += noise
        ...
        with torch.no_grad():
            ...
            if self._experience_type == 'random_transition':
                ...
                if self._experience_type == 'max_supervise':
                    transitions = [i for i, _ in sorted(zip(transitions, transition_supervise), key=lambda x: x[1], reverse=True)]
                if self._experience_type == 'min_supervise':
                    transitions = [i for i, _ in sorted(zip(transitions, transition_supervise), key=lambda x: x[1])]
