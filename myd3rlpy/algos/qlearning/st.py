...
from d3rlpy.dataset import MDPDataset, Episode, TransitionMiniBatch, Transition, ReplayBufferBase, create_fifo_replay_buffer
from d3rlpy.constants import (
    LoggingStrategy,
)
from d3rlpy.algos.utility import assert_action_space_with_dataset, assert_action_space_with_env, build_scalers_with_env, build_scalers_with_transition_picker
...
class STBase(QLearningAlgoBase):
    ...
    def fit_online(
        ...
        for total_step in xrange(1, n_steps + 1):
            ...
            with logger.measure_time("step"):
                ...
                if total_step > update_start_step and buffer.transition_count > self.batch_size:
                    if total_step % update_interval == 0:
                        ...
                        for _ in range(n_updates):
                            ...
                            for name, val in loss.items():
                                logger.add_metric(name, val)
                    if logging_strategy == LoggingStrategy.STEPS and total_step % logging_steps == 0:
            ...
            if epoch > 0 and total_step % n_steps_per_epoch == 0:
                if eval_env:
                    eval_scorer = evaluate_qlearning_with_environment(
                            self,
                            eval_env,
                            n_trials=eval_n_trial,
