from typing import Any, Dict, Optional, Sequence, List, Union, Callable, Tuple, Generator
    ...
    * TD3 updates the policy function after several Q function updates in order
    ...
    def __init__(
        ...
        *,
        ...
        replay_phi: bool = True,
        replay_psi: bool = True,
        batch_size: int = 256,
        n_frames: int = 1,
        n_steps: int = 1,
        gamma: float = 0.99,
    ...
    def _update(self, batch: TransitionMiniBatch, task_id: int, replay_batches: Dict[str, List[Tensor]], all_data: MDPDataset) -> Dict[str, float]:
        assert self._impl is not None, IMPL_NOT_INITIALIZED_ERROR

        metrics = {}

        # 更新phi和psi。
        half_batch_len = len(batch) // 4
        batch1 = TransitionMiniBatch(batch[: half_batch_len])
        batch2 = TransitionMiniBatch(batch[half_batch_len :])

        phi_loss = self._impl.update_phi(batch1, batch2, task_id)
        ...
        psi_loss = self._impl.update_psi(batch1, batch2, task_id)
        ...
        critic_loss = self._impl.update_critic(batch, task_id, all_data) * self.critic_alpha()
        metrics.update({"critic_loss": critic_loss})

        # delayed policy update
        if self._grad_step % self._update_actor_interval == 0:
            actor_loss = self._impl.update_actor(batch, task_id, all_data) * self.actor_alpha()
    ...
    def fit(
        ...
        replay_datasets: Optional[List[TensorDataset]],
        task_id: int,
        all_data: MDPDataset,
        ...
        replay_eval_episodess: Optional[List[TensorDataset]] = None,
        save_interval: int = 1,
        scorers: Optional[
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if replay_dataloaders is not None:
                ...
                for replay_dataloader in replay_dataloaders:
                    replay_iterators.append(iter(replay_dataloader))
            else:
                ...
                with logger.measure_time("step"):
                    ...
                    with logger.measure_time("sample_batch"):
                        ...
                        if replay_iterators is not None:
                            ...
                            replay_batches = dict()
                            for replay_iterator_num, replay_iterator in (replay_iterators):
                                try:
            ...
            if replay_scorers and replay_eval_episodess:
                if replay_iterators is not None:
                    for replay_num, replay_eval_episodes in enumerate(replay_eval_episodess):
                        # 重命名
