    ...
    * TD3 updates the policy function after several Q function updates in order
    ...
    def make_iterator(self, dataset, replay_dataset, n_steps, n_steps_per_epoch, n_epochs, shuffle):
        if replay_dataset is not None:
            ...
            # replay_dataloader = DataLoader(replay_dataset, batch_size=self._batch_size, shuffle=True, num_workers=0)
            replay_dataloader = DataLoader(replay_dataset, batch_size=self._batch_size, num_workers=0)
                ...
                with logger.measure_time("step"):
                    ...
                    with logger.measure_time("sample_batch"):
                        ...
                        if replay_dataloader is not None:
                            ...
                            replay_idx = torch.randint(0, replay_dataset_length, (self._batch_size,))
                            replay_batch = replay_dataset[replay_idx]
                            # replay_batch = replay_dataset(replay_dataset)
                            # replay_iterator = iter(replay_dataloader)
                        else:
                            replay_batch = None
                          # replay_iterator = None
