...
class STImpl(STImpl, IQLImpl):
    ...
    def _build_critic(self) -> None:
        ...
        self._q_func = ParallelizedEnsembleFlattenMLP(self._n_ensemble, [256, 256], self._observation_shape[0] + self._action_size, 1, device=self.device)
    ...
    def _compute_weight(self, observations, actions) -> torch.Tensor:
        ...
        q_t, _ = torch.max(self._targ_q_func(observations, actions), dim=0)
    ...
    def _compute_critic_loss(
        ...
        assert self._q_func is not None
        value, _ = torch.max(self._q_func(batch.observations, batch.actions), dim=0)
