...
declare -a expectile=('0.7' '0.7' '0.9')
declare -a expectile_min=('0.7' '0.7' '0.9')
declare -a expectile_max=('0.9' '0.7' '0.9')
        ...
        do
                ...
                do
                        ...
                        do
                                for m in "${!mix_type[@]}"
                                do
                                        echo "python continual_single.py --algo ${algo[$k]} --experience_type random_episode --dataset ${dataset[$i]} --dataset_nums ${dataset_nums[$j]} --max_save_num ${max_save_num[$l]} --critic_replay_type ${critic_replay_type[$l]} --critic_replay_lambda ${critic_replay_lambda[$l]} --actor_replay_type ${actor_replay_type[$l]} --actor_replay_lambda ${actor_replay_lambda[$l]} --clone ${clone[$l]} --mix_type ${mix_type[$m]} --expectile ${expectile[$i]} --expectile_min ${expectile_min[$i]} --expectile_max ${expectile_max[$i]} --read_policy 0 --seed ${seed[$s]} > single_output_files/output_${algo[$k]}_${expectile[$i]}_${expectile_min[$i]}_${expectile_max[$i]}_${dataset[$i]}_${dataset_nums[$j]}_${critic_replay_type[$l]}_${critic_replay_lambda[$l]}_${actor_replay_type[$l]}_${actor_replay_lambda[$l]}_${clone[$l]}_${mix_type[$m]}_${max_save_num[$l]}_${seed[$s]}_\$(date +%Y%m%d%H%M%S).txt --gpu \$1" > single_run_files/run_${algo[$k]}_${expectile[$i]}_${expectile_min[$i]}_${expectile_max[$i]}_${dataset[$i]}_${dataset_nums[$j]}_${critic_replay_type[$l]}_${critic_replay_lambda[$l]}_${actor_replay_type[$l]}_${actor_replay_lambda[$l]}_${clone[$l]}_${mix_type[$m]}_${max_save_num[$l]}_${seed[$s]}.sh
                                done
                        done
                done
        done
