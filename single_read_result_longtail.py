...
if __name__ == '__main__':
    ...
    task_nums = ["300_0_0_0_0_0_0"]
    # max_save_nums = ['1000', '10000', '100000']
    clone_actors = ['', 'cloneactor']
    max_save_nums = ['25', '50', '75', '100']
    colors = ['#226E9C', '#045275', '#7CCBA2', '#FF1F5B', '#E9002D', '#00CD6C', '#00B000', '#9CCEA7']
    ...
    for algo, weight_temp, expectile, experience, continual_type, (dataset, task_num), max_save_num, entropy_time, clone_actor in product(algos, weight_temps, expectiles, experiences, continual_types, zip(datasets, task_nums), max_save_nums, entropy_times, clone_actors):
        ...
        if 'iqln' not in algo and 'sacn' not in algo and 'edac' not in algo:
            ...
            algo_file = algo
        item_name = algo_file + '_' + weight_temp + '_' + expectile + '_0.7_0.7_' + dataset + '_' + task_num + '_50000_' + continual_type + '_' + entropy_time + '_' + clone_actor + '_' + experience + '_' + max_save_num#  + '_clearnetwork'
    ...
    for max_save_num in max_save_nums:
        if max_save_num not in results.keys():
            print(f"{max_save_num=}")
        if max_save_num in results.keys():
            real_acc = results[max_save_num][-1]
            x = np.arange(len(real_acc))
            plt.plot(x, real_acc, label=algo.split('_')[0], c=colors[max_save_num])
    plt.legend(loc="lower left")
    plt.xlabel("Train Process")
    plt.ylabel("Reward")
    # save_path = f"pictures/result_longtail_clearnetwork.png"
    save_path = f"pictures/result_longtail.png"
