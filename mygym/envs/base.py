...
class RandomEnv(MetaEnv, MujocoEnv):
    ...
    def __init__(self, observation_shape, log_scale_limit, file_name, *args, rand_params=RAND_PARAMS, **kwargs):
        MujocoEnv.__init__(self, file_name, 4, observation_shape)
    ...
    def set_task(self, task):
        for param, param_val in task.items():
            ...
            if len(param_variable.shape) == 1 and len(param_val.shape) == 2:
                param_val = param_val.squeeze()
            assert param_variable.shape == param_val.shape, f"{param_variable.shape}, {param_val.shape}"
            exec(f"self.model.{param}[:] = param_val")
