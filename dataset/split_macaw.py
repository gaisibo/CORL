...
def split_macaw(top_euclid, dataset_name, inner_paths, envs, include_goal=False, multitask=False, one_hot_goal=False, ask_indexes=False, device='cuda:0'):
    ...
    if dataset_name in ['ant_dir_expert', 'ant_dir_medium', 'ant_dir_random', 'ant_dir_medium_random', 'ant_dir_medium_expert', 'ant_dir_medium_replay']:
		...
		obs_pad_shape = [env_.observation_space.shape[0] for env_ in env]
		act_pad_shape = [env_.action_space.sample.shape[0] for env_ in env]
    else:
...
def split_gym(top_euclid, dataset_name, task_datasets, env, compare_dim=3, ask_indexes=False, device='cuda:0', obs_pad_shape: int = 0, act_pad_shape: int = 0):
    ...
    for dataset_num, dataset in task_datasets.items():
        ...
        if obs_pad_shape > 0:
            observations = np.concatenate([observations, np.zeros([observations.shape[0], obs_pad_dim - observations.shape[1]], dtype=np.float32)], axis=1)
        if act_pad_shape > 0:
            actions = np.concatenate([dataset.actions, np.zeros([dataset.actions.shape[0], act_pad_dim - dataset.actions.shape[1]], dtype=np.float32)], axis=1)
        else:
            actions = dataset.actions
        origin_task_datasets[dataset_num] = MDPDataset(observations, actions, dataset.rewards, dataset.terminals, dataset.episode_terminals)
    ...
    return origin_task_datasets, taskid_task_datasets, indexes_euclids, distances_euclids, env, real_action_size, real_observation_size, obs_pad_dim, act_pad_dim
