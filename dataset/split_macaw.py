...
def split_gym(top_euclid, dataset_name, task_datasets, env, compare_dim=3, ask_indexes=False, device='cuda:0'):
    ...
    for dataset_num, dataset in task_datasets.items():
        if ask_indexes:
            ...
            if not found:
                indexes_euclid, distances_euclid = similar_euclid(dataset.observations, dataset.observations, dataset_name, indexes_name, distances_name, compare_dim=compare_dim, device=device)
