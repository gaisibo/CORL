...
if __name__ == '__main__':
    ...
    max_save_nums = ['10', '25', '50', '75', '100']
    colors = ['#045275', '#9CCB86', '#E9E29C', '#E88471', '#7C1D6F']
    ...
    if not os.path.isdir('single_output_files'):
        ...
        # assert False
    results = dict()
    for algo, ensemble, weight_temp, expectile, experience, continual_type, (dataset, task_num), max_save_num, entropy_time, clone_actor in product(algos, ensembles, weight_temps, expectiles, experiences, continual_types, zip(datasets, task_nums), max_save_nums, entropy_times, clone_actors):
        item_name = algo + '_' + ensemble+ '_' + weight_temp + '_' + expectile + '_0.7_0.7_' + dataset + '_' + task_num + '_50000_' + continual_type + '_' + entropy_time + '_' + clone_actor + '_' + experience + '_' + max_save_num
        ...
        if len(mean_lasts) > 0:
            ...
            with open(f'single_result_files/results.txt', 'a') as fw:
                ...
                print(f'var_bwt: {var_bwt}', file=fw)
                print('', file=fw)
            results[max_save_num] = (mean_last, mean_acc, mean_bwt, var_last, var_acc, var_bwt, real_accs)
    ...
    plt.rc("legend")
    for max_save_num in max_save_nums:
        if max_save_num in results.keys():
            real_acc = results[max_save_num][-1]
            x = np.arange(len(real_acc))
            plt.plot(x, real_acc, label="save trajectories: " + max_save_num, c=colors[max_save_num])
    plt.legend(loc="upper left")
    plt.xlabel("Train Process")
    plt.ylabel("Reward")
    save_path = f"pictures/result_1000_save.png"
